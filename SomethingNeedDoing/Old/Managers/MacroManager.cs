...
using SomethingNeedDoing.Old;
using SomethingNeedDoing.Old.Macros.Commands;
...
internal partial class MacroManager : IDisposable
{
    ...
    private async void EventLoop()
    {
        ...
        while (!token.IsCancellationRequested)
        {
            ...
            catch (Exception ex)
            {
                ...
                Old.Service.ChatManager.PrintError("Peon has died unexpectedly.");
                macroStack.Clear();
                PlayErrorSound();
            }
        }
    ...
    private async Task<bool> ProcessMacro(ActiveMacro macro, CancellationToken token, int attempt = 0)
    {
        ...
        try
        {
            ...
            //        Old.Service.ChatManager.PrintError($"Failed to execute macro step \"{step.Text}\": {ex.Message}");
            //        return;
            //    }
        ...
        {
            Svc.Chat.PrintColor($"{ex.Message}", ex.Color);
            pausedWaiter.Reset();
            PlayErrorSound();
            return false;
        }
        catch (MacroActionTimeoutError ex)
        {
            ...
            if (attempt < maxRetries)
            {
                ...
                Old.Service.ChatManager.PrintError(message);
                attempt++;
        ...
        {
            Old.Service.ChatManager.PrintError($"Failure while running script: {ex.Message}");
            pausedWaiter.Reset();
            PlayErrorSound();
            return false;
        }
        ...
        {
            Old.Service.ChatManager.PrintError($"Failure while running {step} (step {macro.StepIndex + 1}): {ex.Message}");
            pausedWaiter.Reset();
            PlayErrorSound();
            return false;
        }
...
internal sealed partial class MacroManager
{
    ...
    private void EnqueueMacroInternal(MacroNode node)
    {
        ...
        {
            Old.Service.ChatManager.PrintError($"{ex.Message}");
        }
        catch (Exception ex)
        {
            Old.Service.ChatManager.PrintError($"Unexpected error");
